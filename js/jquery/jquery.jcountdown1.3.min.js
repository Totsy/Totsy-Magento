...
function getTimerHtml(event, endCount){
	var currentTime = new Date();
	var endDate = new Date(endCount);
	var diff = endDate.getTime() - currentTime.getTime();
	if (event=='live'){
		if ( (diff/1000) < 60 ) {
			htmlTemplate = "<span class='prefix'>Sale Ends:</span> %{s} Seconds";
		} else if ( (diff/1000) < 3600 ) {
			htmlTemplate = "<span class='prefix'>Sale Ends:</span> %{m} Minutes, %{s} Seconds";
        } else if ( ((diff/1000) > 3600) && ((diff/1000) < 86400) ) {
			htmlTemplate = "<span class='prefix'>Sale Ends:</span> %{h} Hours, %{m} Minutes";
		} else if ( ((diff/1000) > 86400) && ((diff/1000) < 172800) ) {
			htmlTemplate = "<span class='prefix'>Sale Ends:</span> %{d} Day, %{h} Hours";
		} else {
			htmlTemplate = "<span class='prefix'>Sale Ends:</span> %{d} Days, %{h} Hours";
		}
	}else if(event=='upcoming'){
		if( (diff/1000) < 86400 ){
			htmlTemplate = "<span>Opens in</span> %{h}<span class=\"cd-time\">:</span>%{m}<span class=\"cd-time\">:</span>%{s}<span class=\"cd-time\"></span>";
		}else {
			htmlTemplate = "<span>Opens in</span><span id=\"cd-day\"> %{d}</span> <span class=\"cd-day\">Days</span> %{h}<span class=\"cd-time\">:</span>%{m}<span class=\"cd-time\">:</span>%{s}<span class=\"cd-time\"></span>";
		}
	}
	return htmlTemplate;
}

function getEarlyAccessTimerHtml(event, endCount){
    var currentTime = new Date();
    var endDate = new Date(endCount);
    var diff = endDate.getTime() - currentTime.getTime();
    if (event=='live'){
        if ( (diff/1000) < 60 ) {
            htmlTemplate = "<span class='prefix'></span> %{s} Seconds";
        } else if ( (diff/1000) < 3600 ) {
            htmlTemplate = "<span class='prefix'></span> %{m} Minutes, %{s} Seconds";
        } else if ( ((diff/1000) > 3600) && ((diff/1000) < 86400) ) {
            htmlTemplate = "<span class='prefix'></span> %{h} Hours, %{m} Minutes";
        } else if ( ((diff/1000) > 86400) && ((diff/1000) < 172800) ) {
            htmlTemplate = "<span class='prefix'></span> %{d} Day, %{h} Hours";
        } else {
            htmlTemplate = "<span class='prefix'></span> %{d} Days, %{h} Hours";
        }
    }else if(event=='upcoming'){
        if( (diff/1000) < 86400 ){
            htmlTemplate = "%{h}<span class=\"cd-time\">:</span>%{m}<span class=\"cd-time\">:</span>%{s}<span class=\"cd-time\"></span>";
        }else {
            htmlTemplate = "<span id=\"cd-day\"> %{d}</span> <span class=\"cd-day\">Days</span> %{h}<span class=\"cd-time\">:</span>%{m}<span class=\"cd-time\">:</span>%{s}<span class=\"cd-time\"></span>";
        }
    }else if(event=='earlyaccess') {
        if( (diff/1000) < 86400 ){
            htmlTemplate = "%{h}<span class=\"cd-time\">h:</span>%{m}<span class=\"cd-time\">m:</span>%{s}<span class=\"cd-time\">s</span>";
        }else {
            htmlTemplate = "<span id=\"cd-day\"> %{d}</span> <span class=\"cd-day\">Days</span> %{h}<span class=\"cd-time\">h:</span>%{m}<span class=\"cd-time\">m:</span>%{s}<span class=\"cd-time\">s</span>";
        }
    }
    return htmlTemplate;
}
