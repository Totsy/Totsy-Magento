...
function getTimerHtml(event, endCount){
	...
	if (event=='live'){
        ...
        if( (diff/1000) < 86400 ){
            htmlTemplate = "<span>Sale begins in</span> %{h}<span class=\"cd-time\">:</span>%{m}<span class=\"cd-time\">:</span>%{s}<span class=\"cd-time\"></span>";
        }else {
            htmlTemplate = "<span>Sale begins in</span><span id=\"cd-day\"> %{d}</span> <span class=\"cd-day\">Days</span> %{h}<span class=\"cd-time\">:</span>%{m}<span class=\"cd-time\">:</span>%{s}<span class=\"cd-time\"></span>";
        }
    }
	return htmlTemplate;
}
