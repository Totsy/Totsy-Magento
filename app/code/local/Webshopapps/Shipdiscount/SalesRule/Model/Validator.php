            ...
            switch ($rule->getSimpleAction()) {
                ...
                case Mage_SalesRule_Model_Rule::BUY_X_GET_Y_ACTION:
                 ...
                 case Webshopapps_Shipdiscount_SalesRule_Model_Rule::BY_SHIP_PERCENT_ACTION:
            		$cartRules = $address->getCartFixedRules();
                   	$shipDiscountAmount =$address->getShippingAmount()*($rule->getDiscountAmount()/100); 
            		
                   if (!isset($cartRules[$rule->getId()])) {
                        $cartRules[$rule->getId()] = $shipDiscountAmount;
                    }
                    ...
                    if ($cartRules[$rule->getId()] > 0) {
                        if ($this->_rulesItemTotals[$rule->getId()]['items_count'] <= 1) {
                            ...
                            $discountRate = $baseItemPrice * $qty /
                                            $this->_rulesItemTotals[$rule->getId()]['base_items_price'];
                            $maximumItemDiscount = $shipDiscountAmount * $discountRate;
                            $quoteAmount = $quote->getStore()->convertPrice($maximumItemDiscount);
                 ...
                 case Webshopapps_Shipdiscount_SalesRule_Model_Rule::BY_SHIP_FIXED_ACTION:
                    $cartRules = $address->getCartFixedRules();
                   	$shipDiscountAmount = $rule->getDiscountAmount(); 
                   ...
                   if (!isset($cartRules[$rule->getId()])) {
                    ...
                    if ($cartRules[$rule->getId()] > 0) {
                        if ($this->_rulesItemTotals[$rule->getId()]['items_count'] <= 1) {
                            $quoteAmount = $quote->getStore()->convertPrice($cartRules[$rule->getId()]);
                            $baseDiscountAmount = min($address->getBaseShippingAmount(), $cartRules[$rule->getId()]);
                        } else {
                            $discountRate = $baseItemPrice * $qty /
                                            $this->_rulesItemTotals[$rule->getId()]['base_items_price'];
                            $maximumItemDiscount = $shipDiscountAmount * $discountRate;
                            ...
                            $baseDiscountAmount = min($address->getBaseShippingAmount(), $maximumItemDiscount);
                            $this->_rulesItemTotals[$rule->getId()]['items_count']--;
                        }

                        $discountAmount = min($address->getShippingAmount(), $quoteAmount);
                        $discountAmount = $quote->getStore()->roundPrice($discountAmount);
                        $baseDiscountAmount = $quote->getStore()->roundPrice($baseDiscountAmount);
             	...
             	/*	if ($cartRules[$rule->getId()] > 0) {
                        $discountAmount = $quote->getStore()->convertPrice($cartRules[$rule->getId()]);
                        $baseDiscountAmount = $cartRules[$rule->getId()];
                        $cartRules[$rule->getId()] -= $baseDiscountAmount;
                    }
                    $address->setCartFixedRules($cartRules);
                    //$address->setPercentShipping($rule->getDiscountAmount());
                	//$discountAmount=$address->getShippingAmount()*($rule->getDiscountAmount()/100);
                	//$baseDiscountAmount=$address->getBaseShippingAmount()*($rule->getDiscountAmount()/100);
                	break;*/
            }
