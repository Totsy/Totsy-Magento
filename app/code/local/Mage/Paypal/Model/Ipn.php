...
class Mage_Paypal_Model_Ipn
{
    ...
    {
        /*
         * 2013-03-14 CJD - Implemented patch from
         * http://www.bigdune.net/geek-blog/magento/60-magento-paypal-ipn-wrong-order-id.html
         * to help solve Wrong Order ID errors
         */
        $resend = false;
        if (empty($this->_order)) {
            //get the variables we need from the IPN request
            if(array_key_exists('resend', $this->_request)) {
                $resend = $this->_request['resend'];
            }
            $paymentdate = strtotime($this->_request['payment_date']);
            $today = strtotime('now');
            // get proper order
            $id = $this->_request['invoice'];
            $this->_order = Mage::getModel('sales/order')->loadByIncrementId($id);
            if (!$this->_order->getId()) {
                //if this is not the first attempt, check how many days have elapsed
                if($resend && $today->diff($paymentdate)->days >= 2) {
                    //more than two days have elapsed, log an error
                    throw new Exception(sprintf('Wrong order ID: "%s".', $id));
                }
                //this is the first attempt
                else {
                    $this->_debugData['exception'] = sprintf('Order not found ID: "%s".', $id);
                    $this->_debug();
