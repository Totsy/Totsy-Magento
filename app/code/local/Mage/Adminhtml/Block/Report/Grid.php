...
class Mage_Adminhtml_Block_Report_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    public function getCsv()
    {
        ...
        foreach ($this->getCollection()->getIntervals() as $_index=>$_item) {
        	//Hara Song --Start
        	if(!!$email = $this->getCollection()->getCustomerEmail()){
        		$report = $this->getReportWithCustomerEmail($_item['start'], $_item['end'], $email);	
        	}
            else{
    ...
    public function getExcel($filename = '')
    {
        ...
        foreach ($this->getCollection()->getIntervals() as $_index=>$_item) {
        	//Hara Song --Start
        	if(!!$email = $this->getCollection()->getShippingRegion()){
        		$report = $this->getReportWithCustomerEmail($_item['start'], $_item['end'], $email);	
        	}
            else{
            	$report = $this->getReport($_item['start'], $_item['end']);
