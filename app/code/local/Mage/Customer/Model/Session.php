...
class Mage_Customer_Model_Session extends Mage_Core_Model_Session_Abstract
{
    ...
    public function login($username, $password)
    {
        ...
        if ($customer->authenticate($username, $password)) {
            ...
            //Harapartners: For Content Data Separation
            $customerEssentialData = array(
            		'firstname' => $customer->getData('firstname'),
            		'lastname' => $customer->getData('lastname'),
            		'email' => $customer->getData('email')
            );
            $this->setData('customer_info', $customerEssentialData);
            $this->getCookie()->set('validation_status', '1'); // Cookie for CDN logic
    ...
    public function logout()
    {
        if ($this->isLoggedIn()) {
            ...
            //Harapartners: For Content Data Separation
            $this->unsData('customer_info');
            $this->getCookie()->delete('validation_status');
        }
        return $this;
    }
